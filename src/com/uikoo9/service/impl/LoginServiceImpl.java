...
public class LoginServiceImpl implements LoginServiceI{
	...
	public String login(Map<String, String[]> paras, HttpSession session){
		String username = QJfinalUtil.value(paras, QContants.V_USERNAME);
		String password = QJfinalUtil.value(paras, QContants.V_PASSWORD);
		...
		if(QStringUtil.allNotEmpty(new String[]{username,password})){
			...
			if(users != null && users.size() == 1){
				session.setAttribute(QContants.V_USER, users.get(0));
